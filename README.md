...
This is an end-to-end build guide and installation tutorial with full parts/hardware listing, pretrained YOLOv3 weights for "Human head", and ROS packages to build a robotic pan/tilt turret that uses machine learning and relatively inexpensive hardware to autonomously "target" and track human heads.  The code and weights files for this are hosted from a google drive download link included in this readme (simplifies download process), and you can see a few examples of it in action in the gifs below:
